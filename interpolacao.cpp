// from:
// https://github.com/PauloMiranda98/Competitive-Programming-Notebook/blob/54af0a8dcefdeb5505538a3716855db62bcdc716/code/math/lagrange.h#L30
...
void mul(vector<ld> &A, int x0) {  // multiply A(x) by (x - x0)
  ...
  A[0] = 0;
  for (int i = 0; i < n + 1; i++) A[i] -= B[i] * x0;
}
...
vector<ld> LagrangeInterpolation(vector<PointValue> vp) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    for (int j = 0; j < n; j++) ans[j] += A[j] * (y / d);
    mul(A, vp[i].x);
  }
  return ans;
}
