...
struct item {
  ...
  item operator+(item oth) {
    ...
    return {this->color, 
            this->sz + oth.sz, 
            this->sz == this->pref && this->color == oth.color ? this->pref + oth.pref : this->pref,
            0};
  }
};
...
void build(int n){
    ...
    for (int i=0; i < n; i++)
        seg_color.set(i, color[sop[i]]);
    
    light = vector<vi>(n, vi(2));
    seg_dp = segtree<item>(n);
    for (int u=1; u < n; u++) if (head[u] == u){
...
void toggle(int u){
    bool has_changed = 0;
    while (head[u] != 0){
        light[par[head[u]]][color[head[u]]] -= seg_dp.get(pos[head[u]], pos[tail[u]] + 1).pref;
        if (!has_changed){
            has_changed = 1;
            color[u] ^= 1;
...
int solve(int u){
    int c = color[u];
    while (head[u] != 0 && color[par[head[u]]] == c && (seg_color.get(pos[head[u]], pos[u] + 1) == (c * (h[u] - h[head[u]] + 1))))
        u = par[head[u]];
    int l = 0;
    int r = h[u] - h[head[u]] + 1;
    while (r - l > 1){
        int mid = l + (r - l)/2;
        if (seg_color.get(pos[u] - mid, pos[u] + 1) == (c * (mid + 1))) l = mid;
        else r = mid;
    }
    u = sop[pos[u] - l];
